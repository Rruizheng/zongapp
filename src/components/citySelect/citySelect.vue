<template>
 <div id="citySelect">
   <select v-model="selectedProvince" name="province">
     <option v-for="(item, index) in provinces" v-if="item.level === 1" :value="item" :key="index">
       {{ item.name }}
     </option>
   </select>
   ...
   <select v-model="selectedBlock" name="block">
     <option v-for="(item, index) in blocks" :value="item" :key="index">
       {{ item.name }}
     </option>
   </select>
 </div>
...
import provinces from '../../data/provinces-china/provinces.js';
import Vue from 'vue';
/* eslint-disable */
export default {
 name: 'citySelect',
